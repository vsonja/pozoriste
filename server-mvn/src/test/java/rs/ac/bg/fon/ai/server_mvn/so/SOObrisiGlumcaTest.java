...
class SOObrisiGlumcaTest {
	...
	void test() {
		...
		when(dbb.obrisiObjekat(g)).thenReturn(true);
		...
		verify(dbb).obrisiObjekat(g);
        assertEquals("Glumac je uspešno obrisan.", so.getPoruka());
