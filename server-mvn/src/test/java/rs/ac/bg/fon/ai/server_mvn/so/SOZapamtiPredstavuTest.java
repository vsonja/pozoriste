...
class SOZapamtiPredstavuTest {
	
	SOZapamtiPredstavu so;
	DBBroker dbb;

	@BeforeEach
	void setUp() throws Exception {
		so = new SOZapamtiPredstavu();
	...
	void testExecuteOperation() {
		...
		when(dbb.UbaciNoviObjekat(p)).thenReturn(true);
        ...
        verify(dbb).UbaciNoviObjekat(p);
        assertEquals("Predstava je uspesno sacuvana.", so.getPoruka());
