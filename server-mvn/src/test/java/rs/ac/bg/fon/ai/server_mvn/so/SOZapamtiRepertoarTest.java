...
class SOZapamtiRepertoarTest {
	
	SOZapamtiRepertoar so;
	DBBroker dbb;

	@BeforeEach
	void setUp() throws Exception {
		so = new SOZapamtiRepertoar();
	...
	void testExecuteOperation() {
		...
		when(dbb.UbaciNoviObjekat(r)).thenReturn(true);
        ...
        verify(dbb).UbaciNoviObjekat(r);
        assertEquals("Repertoar je uspesno sacuvan.", so.getPoruka());
