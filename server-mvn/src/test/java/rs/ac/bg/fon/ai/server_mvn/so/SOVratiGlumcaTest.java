...
class SOVratiGlumcaTest {
	...
	void setUp() throws Exception {
		so = new SOVratiGlumca(UslovWhere.PRIMARNI_KLJUC);
	...
	void testExecuteOperation() {
		...
		when(dbb.vratiSveObjekteSaUslovom(g, so.uslov)).thenReturn(glumci);
        ...
        verify(dbb).vratiSveObjekteSaUslovom(g, so.uslov);
